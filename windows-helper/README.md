...
Each `/preview` viewer receives every published frame, so opening the built-in
test page does not reduce the clinical app's frame rate. Each additional
1600x1200 viewer does add another full-rate stream, however. Close unused test
pages if the network cannot carry multiple streams. `/still` continues to use
...
| `preview_frames` | Running count of preview frames captured and published inside the helper since this capture session started, including frames produced with no viewer. |
| `preview_clients` | Number of active `/preview` viewers. Every viewer receives every published frame, so each additional viewer adds another full-rate stream; the high-resolution `/still` capture buffer is unaffected. |
